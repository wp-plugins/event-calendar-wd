...
(function ($) {
    ...
    $(document).on('click', '.ecwd-excluded-events .ecwd-calendar-event-add', function () {
        var clicked_el = this;
        var element = $(this).closest('.ecwd-calendar-event');
        var calendar_id = $('#post_ID').val();
        var event_id = $(element).find('input').val();
    ...
    $(document).on('click', '.event_cal_add .close', function(e){
        $('.event_cal_add').hide();
    });
    $(document).on('mouseup', '.day-with-date', function (e) {
        var position = $(this).position();
        $('.event_cal_add').css({"left" :  position.left , "top" : position.top});
    });
